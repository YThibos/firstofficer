...
- Changes to harness adapters (detection in `bin/fm-harness.sh`, launch and hook mechanics in `bin/fm-spawn.sh`, busy signatures in `bin/fm-watch.sh` and `bin/fm-tmux-lib.sh`, cleanup in `bin/fm-teardown.sh`, and per-adapter facts in `.agents/skills/harness-adapters/references/<harness>.md`) must be verified empirically against the real harness, never written from documentation alone.
...
Tracked changes to firstmate itself - `AGENTS.md`, `CLAUDE.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - ship through the `no-mistakes` pipeline on a feature branch and require an explicit merge approval.
...
bin/fm-test-run.sh tests/fm-anchor-budget.test.sh   # the always-loaded anchor is a real file and inside its byte budget
