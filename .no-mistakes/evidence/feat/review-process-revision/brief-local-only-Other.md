...
# Setup
You are in a disposable git worktree of Other, at a detached HEAD on a clean default branch.
...
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/tmp.OEqACYBany/state/t-local-only-Other.status'`
...
6. If a decision belongs above the implementation worker (product choices, destructive actions),
   ...
   For a no-mistakes ask-user gate specifically, escalate all ask-user findings as one event plus one snapshot file, using that same shape even when the gate holds only a single ask-user finding: write only the ask-user findings, verbatim and unparaphrased (id, severity, file, line, description, authority), to `/tmp/tmp.OEqACYBany/data/t-local-only-Other/nm-<run>-findings.txt`, then report the gate with
   `needs-decision [key=nm-<run>-<step>]: ask-user findings=<id1>,<id2>,... file=/tmp/tmp.OEqACYBany/data/t-local-only-Other/nm-<run>-findings.txt`
...
Firstmate steers you through durable message files in '/tmp/tmp.OEqACYBany/state/t-local-only-Other.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/tmp.OEqACYBany/state/t-local-only-Other.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/tmp.OEqACYBany/state/t-local-only-Other.inbox'/NNN.msg '/tmp/tmp.OEqACYBany/state/t-local-only-Other.inbox'/handled/`.
