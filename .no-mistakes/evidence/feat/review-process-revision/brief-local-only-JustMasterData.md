...
# Setup
You are in a disposable git worktree of JustMasterData, at a detached HEAD on a clean default branch.
...
1. First action: create your branch: `git checkout -b feat/x`
...
# Rules
1. Never push to the default branch (publish only your `feat/x` branch, through the pipeline run below). Open the merge request only as a draft, never mark it ready, and never merge.
2. Stay inside this worktree; modify nothing outside it.
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/tmp.OEqACYBany/state/t-local-only-JustMasterData.status'`
...
6. If a decision belongs above the implementation worker (product choices, destructive actions),
   ...
   For a no-mistakes ask-user gate specifically, escalate all ask-user findings as one event plus one snapshot file, using that same shape even when the gate holds only a single ask-user finding: write only the ask-user findings, verbatim and unparaphrased (id, severity, file, line, description, authority), to `/tmp/tmp.OEqACYBany/data/t-local-only-JustMasterData/nm-<run>-findings.txt`, then report the gate with
   `needs-decision [key=nm-<run>-<step>]: ask-user findings=<id1>,<id2>,... file=/tmp/tmp.OEqACYBany/data/t-local-only-JustMasterData/nm-<run>-findings.txt`
...
Firstmate steers you through durable message files in '/tmp/tmp.OEqACYBany/state/t-local-only-JustMasterData.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/tmp.OEqACYBany/state/t-local-only-JustMasterData.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/tmp.OEqACYBany/state/t-local-only-JustMasterData.inbox'/NNN.msg '/tmp/tmp.OEqACYBany/state/t-local-only-JustMasterData.inbox'/handled/`.
The move IS the acknowledgement: without it firstmate rings again and eventually treats you as stuck. An empty or absent inbox needs no action.

# Project memory
If `AGENTS.md` or `CLAUDE.md` already exists, or if this task produced durable project-intrinsic knowledge, run `/home/thibyann001/.no-mistakes/worktrees/04c39aeaee8f/01M3C9E3T5C51VKMVQ8QJM0FPX/bin/fm-ensure-agents-md.sh .` in the worktree.
Record only project knowledge useful to almost every future session.
For anything the codebase already shows, prefer a pointer to the authoritative file, command, or doc over copying the detail.
If you touch a project `AGENTS.md`, follow `/home/thibyann001/.no-mistakes/worktrees/04c39aeaee8f/01M3C9E3T5C51VKMVQ8QJM0FPX/bin/fm-ensure-agents-md.sh`'s self-governance contract in the same pass.
...
Work these stages in order on your branch `feat/x`.
...
If this project has no remote at all, the pipeline has nowhere to publish: skip the draft check and do not run it, append `done: ready in branch feat/x` instead, and the configured merge authority approves before firstmate merges it into the local default branch through the guarded fast-forward path.
