...
#   -h,--help        usage
#
# Output contract: `fm-bearings.v1`. No reports; it reads the session lock but
# never acquires or reclaims one, and the underlying snapshot's parent-side
# remote-ledger cache refresh is the only default fleet-state mutation.
set -u
...
Default fields: schema, home, generated, prs, session_lock,
  in_flight{id,kind,state,repo,name,doing},
