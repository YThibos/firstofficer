...
# and tells you to have the crewmate rebase. It also refuses to "merge" the
# default branch into itself, so a task whose worktree never left the default
# branch can never be reported as landed. See AGENTS.md prime directives,
...
# and merging it into itself would report a landing that never happened.
[ "$BRANCH" != "$DEFAULT" ] || { echo "error: task $ID resolves to the default branch '$DEFAULT'; it has no separate work branch to merge" >&2; exit 1; }
