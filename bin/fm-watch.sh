...
# crew-state read, and the worktree write probe all run ONLY here, inside the
# at-threshold branch that is about to escalate: at most one each per window per
# STALE_ESCALATE_SECS, never per poll. The wait consult runs first, because a pane
# whose worker already said why it is quiet has nothing to prove through its
# pipeline or its worktree.
#
# The crew-state read that follows it exists because a worker blocked on a
