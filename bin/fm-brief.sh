...
# charters and craft-review briefs still use a single `{TASK}` fill. Firstmate may adjust other
...
for a in "$@"; do
  if [ -n "$want_value" ]; then
    case "$a" in
      --*) brief_missing_value "$want_value" ;;
    esac
    case "$want_value" in
...
[ -z "$want_value" ] || brief_missing_value "$want_value"
...
ASK_USER_BLOCK=
# Both pipeline-running modes can raise ask-user gates, so both carry the rule.
if [ "$KIND" = ship ] && { [ "$MODE" = no-mistakes ] || [ "$MODE" = local-only ]; }; then
