...
family_for_basename() {
  case "$1" in
    ...
    fm-update.test.sh|fm-upstream-sync.test.sh)
      printf '%s\n' session-bootstrap
      ;;
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-craft-review.test.sh|fm-merge-local.test.sh|fm-pr-check-security.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-backend.sh|bin/fm-backend-hometag-lib.sh)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    bin/fm-watch*|bin/fm-wake*|bin/fm-limit-*|\
    ...
    bin/fm-sessionstart-nudge.sh|bin/fm-tangle*|bin/fm-update.sh|bin/fm-upstream-sync.sh|\
