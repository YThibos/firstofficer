...
family_for_basename() {
  case "$1" in
    ...
    fm-tangle-guard.test.sh|fm-update.test.sh|fm-upstream-sync.test.sh)
      printf '%s\n' session-bootstrap
      ;;
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-agent-process-lib.sh)
      ...
      printf '%s\n' pure-contract-unit
      ;;
    bin/fm-watch*|bin/fm-wake*|bin/fm-limit-*|bin/fm-inactive-reconcile.sh|\
    ...
    bin/fm-sessionstart-nudge.sh|bin/fm-startup-network.sh|bin/fm-tangle*|bin/fm-update.sh|bin/fm-upstream-sync.sh|\
