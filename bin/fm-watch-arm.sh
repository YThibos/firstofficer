...
# A cycle whose watcher the cycle-exit ledger already records as closing on an
# actionable wake is closed as a handoff first: two arms follow one watcher
# whenever a manual repair overlaps the Stop-owned auto-arm, the owning arm
# already reported that wake, and calling it "supervision is down" told the
# model to repair, which started another competing arm (four consecutive false
# failures in one session, 2026-09-03).
#
