...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  [ -z "$BORROW_WT" ] || { echo "error: --relaunch reuses the task's recorded worktree; --borrow-worktree cannot override it" >&2; exit 1; }
else
...
# (busy-state source, exit command, dialogs, quirks) lives in that adapter's reference
# in the harness-adapters skill.
launch_template() {
  ...
  case "$harness" in
    ...
    # For a crewmate, --settings instead names state/<task-id>.claude-settings.json,
    # which carries the same policy plus the busy-state and turn-end hooks, so the
    # declaration lives in state/ instead of the worktree and two agents can share
    # one checkout without overwriting each other's hook. The file form is
    # deliberate over inline JSON there: this string is assembled in shell, sent
    # through tmux, and re-expanded in the pane, and a path avoids three layers of
    # nested-quote risk. A secondmate has no spawn-written hook - its home is a
    # firstmate checkout whose own tracked .claude/settings.json holds the primary
    # guards - so it carries the policy inline.
    claude)
      if [ "$kind" = secondmate ]; then
        ...
        printf '%s' 'CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude __CLAUDEPERMFLAG__ --settings __CLAUDESETTINGS__ __MODELFLAG____EFFORTFLAG__"$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
      fi
      ;;
    codex)
      if [ "$kind" = secondmate ]; then
...
if [ "$KIND" != secondmate ]; then
  ...
  case "$HARNESS" in
    claude*)
      ...
      # The launch-policy keys ride in the same file, because --settings takes
      # one value and a crewmate's names this file.
