...
That second rule is what bounds an inert auto-arm: a hook kept silent by a missing session lock, a hook that never fires, or a hook failing before its generation claim leaves the ledger frozen at its last outcome.
