...
firstmate's always-loaded operating contract and routing index for conditional procedures is the fork-owned anchor [`CLAUDE.md`](../CLAUDE.md), with [`AGENTS.md`](../AGENTS.md) as the full supervisor contract it reads on demand; this is the human-facing companion.
...
A pane idle at its prompt whose own agent is waiting on a background job it started inside the task's recorded worktree, such as a full test suite or a backgrounded validation drive call, defers the escalation the same way.
That evidence is kernel process structure alone: a shell child of a verified harness process that leads its own process group, has no controlling terminal, runs inside that worktree, and started no more than `FM_BG_JOB_MAX_SECS` ago; `crew_background_job_of` in `bin/fm-classify-lib.sh` owns the test and its rationale.
The same evidence absorbs such a pane's first stale sighting onto the wedge timer instead of surfacing it at once, unless the worker declared a wait, and it defers the away-mode daemon's stale persistence recheck the same way.
A busy pane past `FM_BUSY_TURN_MAX_SECS` is never excused by it, because its own foreground command has the same process shape, and a secondmate is never excused by it, because its home runs its own supervision in background shells.
