...
firstmate's always-loaded operating contract and routing index for conditional procedures is the fork-owned anchor [`CLAUDE.md`](../CLAUDE.md), with [`AGENTS.md`](../AGENTS.md) as the full contract it reads on demand; this is the human-facing companion.
...
On `attached` it stays live across identity-matched successors, and an unexplained clean child close either attaches to a verified healthy successor, closes as a recorded handoff when the arm that owns the watcher already reported that cycle's actionable wake, or becomes the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result.
