...
Before that ledger is consulted, the arm checks for a handoff: two arms follow one watcher whenever a manual repair overlaps the Stop-owned auto-arm, and only the arm that owns the watcher reads the reason it printed.
...
The time bound is what keeps a recycled PID from speaking for a watcher it is not.
