...
The exact selection contract and safety rationale live in [architecture](architecture.md#runtime-session-backends), while the signatures live in each adapter's own reference under [the harness-adapters skill](../.agents/skills/harness-adapters/references).
