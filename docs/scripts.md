...
| `fm-bearings-snapshot.sh` | Project the bounded remote-ledger fleet snapshot to compact TOON, plus a read-only `session_lock` field for this session's own lock condition (docs/session-lock.md); `--include-prs` adds live GitHub enrichment |
...
| `fm-upstream-sync.sh`    | Guarded merge of the original upstream project into this fork, in an isolated sync copy: dated sync branch, conflict classification, and the validated autonomous landing |
...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity, shared-service boundary, and limit-stop takeover test for fm-lock.sh and the Claude Stop auto-arm (docs/session-lock.md) |
...
| `fm-spawn.sh`            | Spawn crewmates, scouts, `id=repo` batches, and secondmates on the resolved harness and runtime backend, optionally joining another task's worktree |
...
| `fm-craft-rules.sh`      | Decide which projects carry the captain's craftsmanship rules in their no-mistakes intent, and print those rules |
...
| `fm-lock.sh`             | Per-home firstmate session lock, including takeover from a usage-limit-stopped holder (docs/session-lock.md) |
