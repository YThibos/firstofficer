...
It also holds `state/<id>.limit-park`, the durable record for a worker its own harness parked on a usage limit: when that parked footer was first seen, which anchors every absolute reset time read from the pane, and how many resume attempts have been spent.
`bin/fm-limit-park-lib.sh` owns the record and removes it once the pane stops showing a parked footer, so a worker that came back never carries a stale anchor or a spent budget into its next park; the watcher's own `.limit-surfaced-*` marker beside it is a watcher internal and is never touched by hand.
...
The verified adapter knowledge - busy signatures, interrupt and exit commands, skill-invocation syntax, and per-harness quirks - lives in one per-adapter reference under [`.agents/skills/harness-adapters/references/`](../.agents/skills/harness-adapters/references), selected by the recorded `harness=` value.
...
When a running home advances and its loaded instruction surface (`CLAUDE.md`, `AGENTS.md`, `bin/`, or `.agents/skills/`) changed, bootstrap sends the re-read nudge itself through the stable `fm-<id>` selector and reports the exact completed send as `BOOTSTRAP_INFO:`.
...
FM_LIMIT_PARK_MAX_WAIT=21600       # seconds a parked worker may be waited out before a resume is attempted whatever the footer claims; bounds an unreadable or counting-down reset time, and never overrides bin/fm-limit-resume.sh's own refusal while the pane still names a future reset
FM_LIMIT_RESUME_RETRY_SECS=300     # seconds between resume attempts on the same park
FM_LIMIT_RESUME_MAX_ATTEMPTS=3     # resume attempts per park before the worker is reported for a manual steer; refusals (not parked, not reset yet) spend none of it
FM_LIMIT_RESUME_MESSAGE=           # optional single-line override for the resume message; never a bare keystroke, which does not resume such a worker
FM_LIMIT_RESUME_VERIFY_TRIES=5     # pane re-reads after the resume message before it counts as not landed
FM_LIMIT_RESUME_VERIFY_SLEEP=1     # seconds between those re-reads
