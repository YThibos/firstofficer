...
4. **In explicit file mode only, compose and replace the detailed report file.**
   ...
   - **Charted Next** - queued or gated work, including deferred or aged captain-hold safety gates, any main-inventory integrity warning, and any reportable `session_lock` condition, with each item's blocker, date, age, or integrity reason.
