...
description: >-
  ...
  Fast-forwards this firstmate repo's default branch and every local or remote secondmate through its guarded update path (never forced, never disruptive), then re-reads its CLAUDE.md anchor and restarts every live second mate through the persist-gated restart, with a fallback re-read nudge only where a restart cannot be proven.
user-invocable: true
metadata:
  internal: true
---
...
Firstmate is its own repo, behind the same no-mistakes gate as any project, so new tracked material (`CLAUDE.md`, `AGENTS.md`, `bin/`, `.agents/skills/`, and public `skills/`) reaches `main` and then sits there until each running firstmate pulls it.
Only `CLAUDE.md`, `AGENTS.md`, `bin/`, and `.agents/skills/` are a running firstmate instruction surface; public `skills/` is installer-facing and is not loaded by firstmate.
`CLAUDE.md` is the fork-owned anchor a running firstmate actually loads, and `AGENTS.md` is the full upstream contract it reads on demand.
...
A running agent holds its `CLAUDE.md` anchor and every skill it has already loaded frozen from the moment it launched, and no verified harness offers a reload, so new bytes on disk change nothing for it until it starts a fresh conversation.
...
2. **Re-read your instructions if they changed.**
   When the updater printed `reread-firstmate: yes`, the tracked instruction surface (`CLAUDE.md`, `AGENTS.md`, `bin/`, or `.agents/skills/`) just advanced under you.
   **Read `CLAUDE.md` now** - it is the anchor you operate from - to refresh your operating instructions before doing anything else, so you are acting on the new instructions rather than the stale ones you were started with.
   Read the matching `AGENTS.md` section as well when the change points you at it.
...
4. **Send the re-read message to the rest.**
   ...
   ```sh
   FM_HOME=<this-firstmate-home> bin/fm-send.sh <id> 'firstmate was updated to the latest - please re-read your CLAUDE.md to pick up the new instructions.'
   ```
...
5. **Report to the captain in plain outcomes, in one line where you can.**
   Summarize what landed under section 9 of your anchor without firstmate's internal vocabulary: which parts of the fleet are now on the latest, and which were left as-is and why.
