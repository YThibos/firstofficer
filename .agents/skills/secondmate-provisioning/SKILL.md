...
No instruction reread nudge is needed at spawn or respawn because the agent reads instructions fresh on launch; only the bootstrap sweep's running-home instruction-surface advance needs that re-read, and the nudge points the secondmate at its always-loaded `CLAUDE.md` anchor.
Bootstrap reports successful instruction re-read sends as `BOOTSTRAP_INFO:` and only emits `NUDGE_SECONDMATES:` when that send fails and needs retry.
