...
A new adapter's parked-by-usage-limit footer signature in `../../../bin/fm-limit-park-lib.sh` is registered only once that harness's parked pane has actually been observed; there is no shared default, and a harness with no registered signature never classifies as parked, so leaving it out is safe.
