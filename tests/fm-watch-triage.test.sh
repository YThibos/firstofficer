...
test_deferral_clears_the_escalation_count() {
  ...
  reap "$pid"
  # The reaped watcher leaves its stopped cycle durable until handled; acknowledge
  # it so the next watcher starts from a clean queue rather than replaying it.
  ack_stopped_cycle "$state" || { unset FM_FAKE_PIPELINE_LIVENESS; fail "could not acknowledge the intentional first-phase stop"; }
  ...
  grep -F "possible wedge" "$out" >/dev/null || { unset FM_FAKE_PIPELINE_LIVENESS; fail "a stopped pipeline did not flag a possible wedge after the deferral: $(cat "$out")"; }
