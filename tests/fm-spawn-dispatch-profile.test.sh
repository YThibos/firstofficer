...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  expected="env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --settings '$HOME_DIR/state/$id.claude-settings.json' $CLAUDE_CONTROL_CHANNEL_FLAG \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < '$HOME_DIR/data/$id/launch-brief.md')\""
...
test_claude_threads_model_and_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "claude --dangerously-skip-permissions --settings '$HOME_DIR/state/$id.claude-settings.json' $CLAUDE_CONTROL_CHANNEL_FLAG --model 'sonnet' --effort 'high'" \
...
test_claude_crewmate_launch_carries_the_attribution_policy() {
  ...
  expect_code 0 "$status" "claude crewmate spawn should succeed"$'\n'"$out"
  launch=$(cat "$LAUNCH_LOG")
  # A crewmate's launch names its own per-task settings file rather than inline
  # JSON, so two agents can share one checkout; the policy rides in that file.
  settings="$HOME_DIR/state/$id.claude-settings.json"
  assert_contains "$launch" "--settings '$settings'" "claude crewmate launch does not name its own settings file"
  assert_attribution_policy "$(cat "$settings")" "claude crewmate"
  assert_contains "$(cat "$settings")" '"feedbackDrafts":"off"' "claude crewmate settings do not disable feedback drafts"
...
claude_expected_launch() {  # <home> <id> <permission-flag>
  ...
  printf '%s' "env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude $flag --settings '$home/state/$id.claude-settings.json' $CLAUDE_CONTROL_CHANNEL_FLAG \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < '$home/data/$id/launch-brief.md')\""
}
