...
STUB
  ...
  for mode in no-mistakes direct-PR local-only; do
    ...
    # A brief takes its branch from the caller, and promotion's is fm/<id>, so the
    # ordinary brief is given that same branch for a like-for-like comparison.
    FM_HOME="$home" "$BRIEF" "$id" fixture-project --mode "$mode" --branch "fm/$id" >/dev/null 2>&1 \
